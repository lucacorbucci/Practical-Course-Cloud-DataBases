...
public class FileMap implements FileMapInterface {
     ...
     * */
    public FileMap(String fileName, int MaxSize, Path path){
    ...
    public FileMap(int maxSize, ArrayList<Pair<String, String> > list, Path path){
        ...
        for(Pair<String, String> p : list){
            hashMap.putIfAbsent(p.getKey(), p.getValue());
        }
     ...
     */
    private int getLastHash(HashMap<String, String> hMap){
        ArrayList<Pair<String, Integer>> hashes = new ArrayList<>();
        hMap.forEach((key, value) -> hashes.add(new Pair<String, Integer>(key, key.hashCode())));
        hashes.sort((o1, o2) -> -Integer.compare(o2.getValue(), o1.getValue()));
        return hashes.get(hMap.size()-1).getValue();
    }
    ...
    private rebalanceReturn rebalance(HashMap<String, String> hMap){
        ...
        int firstHash = hashes.get(index).getValue();
        int lastHash = hashes.get(hMap.size()-1).getValue();

        // First part of the elements
        List<Pair<String, Integer>> head = hashes.subList(0, index);
     ...
     */
    private void removeInBatch(HashMap<String, String> hMap, List<Pair<String, Integer>> hash){
        for(Pair<String, Integer> h : hash){
            hMap.remove(h.getKey());
        }
        write(hMap);
    }
     ...
     */
    private HashMap<String, String> read(String fileName){
        HashMap<String, String> hMap = null;
        try {
