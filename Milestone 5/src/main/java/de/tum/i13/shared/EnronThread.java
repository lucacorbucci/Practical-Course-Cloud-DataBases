...
public class EnronThread implements Runnable {
    ...
    EnronThread(ConcurrentLinkedQueue<Pair<String, String>> queue) {
        ...
        while (!fine) {
            ...
            if (pair != null) {
                if (pair.getFirst().equals("-111")) {
                    ...
                    kvs.putKV(ac, new String[]{"put", trimByBytes(pair.getFirst(), 20), trimByBytes(pair.getSecond(), 120000)});
                    try {
                        ac.close();
                    } catch (Exception e) {
                        e.printStackTrace();
        ...
        while (!fine) {
            ...
            if (pair != null) {
                if (pair.getFirst().equals("-111")) {
                    ...
                    kvs.getValue(ac, new String[]{"get", trimByBytes(pair.getFirst(), 20)});
                    try {
                        ac.close();
                    } catch (Exception e) {
                        e.printStackTrace();
    ...
    private static ActiveConnection connectToServer(int port) {
        ...
        //ac = kvs.buildConnection(new String[]{"connect", "116.203.183.105", String.valueOf(5000)});
